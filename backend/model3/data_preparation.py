        ...
        with tqdm(total=min(total_frames // sample_rate, max_frames or float('inf'))) as pbar:
            while cap.isOpened():
                ...
                if frame_num % sample_rate == 0:
                    ...
                    cv2.imwrite(output_path, frame, [cv2.IMWRITE_JPEG_QUALITY, 98])
                    extracted += 1
                    pbar.update(1)
...
class ShuttleAnnotationTool:
    ...
    def annotate(self):
        ...
        while self.current_idx < len(self.images):
            ...
            self.scale_x = self.INPUT_WIDTH / w
            self.scale_y = self.INPUT_HEIGHT / h

            display_img = cv2.resize(img, (self.INPUT_WIDTH, self.INPUT_HEIGHT))
            
            # Show existing annotation if available
            if image_name in self.annotations:
                x, y = self.annotations[image_name]
                sx, sy = int(x * self.scale_x), int(y * self.scale_y)
            ...
            if self.current_point:
                cv2.circle(display_img, (int(self.current_point[0] * self.scale_x),
                         int(self.current_point[1] * self.scale_y)),
                         5, (255, 0, 0), -1)
    ...
    def _save_annotations(self):
        ...
        for image_name, point in self.annotations.items():
            ...
            if point is None:
                ...
                cv2.imwrite(os.path.join(self.image_dir, image_name), img)
                x_center = x / w
                y_center = y / h
                width = 20 / w
                height = 20 / h
...
class TrackNetDataPreparator:
    ...
    @staticmethod
    def frames_to_tracknet_format(images_dir, annotations_json, output_path):
        """
        ...
        for frame_name, value in annotations.items():
            ...
            if not isinstance(value, (list, tuple)) or len(value) != 2:
                print(f"Skipping invalid annotation for {frame_name}: {value}")
                continue
            
            x, y = value

            # Read frame from path
            ...
            # Normalize
            processed_annotations[frame_num] = (
                x / w,
                y / h
            )
...
if __name__ == "__main__":
    ...
    if args.action == 'extract':
        ...
        TrackNetDataPreparator.frames_to_tracknet_format(
